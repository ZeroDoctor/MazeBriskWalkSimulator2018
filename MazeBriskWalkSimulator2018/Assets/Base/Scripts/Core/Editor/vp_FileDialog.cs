...
public class vp_FileDialog : EditorWindow
{
	...
	public static void Create(Mode mode, string caption, string path, Callback callback, string extension)
	{
		...
		vp_FileDialog dialog = (vp_FileDialog)EditorWindow.GetWindow(typeof(vp_FileDialog), true);
		if (string.IsNullOrEmpty(caption))
		{
			if (m_Mode == Mode.Open)
				dialog.titleContent.text = "Open";
			if (m_Mode == Mode.Save)
				dialog.titleContent.text = "Save As";
		}
		else
			dialog.titleContent.text = caption;
